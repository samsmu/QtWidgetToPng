...
function (pvs_studio_analyze_file SOURCE SOURCE_DIR BINARY_DIR)
    ...
    if (EXISTS "${SOURCE}" AND NOT TARGET "${LOG}" AND NOT "${PVS_STUDIO_LANGUAGE}" STREQUAL "")
        add_custom_command(OUTPUT "${LOG}"
                           COMMAND touch "${PVS_STUDIO_CONFIG}"
                           COMMAND mkdir -p "${PARENT_DIR}"
                           COMMAND rm -f "${LOG}"
                           COMMAND ${PVS_STUDIO_BIN} --output-file "${LOG}"
                                                     --source-file "${SOURCE}"
                                                     ${PVS_STUDIO_ARGS}
...
function (pvs_studio_add_target)
    ...
    set(SINGLE LICENSE CONFIG TARGET LOG FORMAT BIN CONVERTER PLATFORM PREPROCESSOR)
    ...
    default(PVS_STUDIO_C_FLAGS "${CMAKE_C_FLAGS}")
    default(PVS_STUDIO_CXX_FLAGS "${CMAKE_CXX_FLAGS}")
    default(PVS_STUDIO_TARGET "pvs")
    default(PVS_STUDIO_LOG "PVS-Studio.log")
    default(PVS_STUDIO_BIN "pvs-studio")
    ...
    add_custom_target("${PVS_STUDIO_TARGET}" ${ALL} ${COMMANDS} DEPENDS ${PVS_STUDIO_DEPENDS} "${PVS_STUDIO_LOG}")
endfunction ()
